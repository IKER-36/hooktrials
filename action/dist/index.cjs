...
function parseTrialConfig(source, environment = process.env) {
  const raw = record(import_yaml.default.parse(source), "config");
  ...
  const attempts = raw.attempts.map((item, index) => {
    const attempt = typeof item === "number" ? { expect: item } : record(item, `attempts[${index}]`);
    return {
      expect: integer(attempt.expect, `attempts[${index}].expect`, 100, 599),
      waitMs: integer(attempt.waitMs ?? 0, `attempts[${index}].waitMs`, 0, 3e4)
    };
  });
  const headerValues = raw.headers === void 0 ? {} : record(raw.headers, "headers");
  ...
  return {
    ...
    eventId: typeof raw.eventId === "string" && raw.eventId.trim() ? raw.eventId.trim() : `ht-ci-${Date.now()}`,
    timeoutMs: integer(raw.timeoutMs ?? 15e3, "timeoutMs", 1e3, 6e4),
    attempts
  };
}
async function runTrial(config, fetcher = fetch) {
...
function junitReport(result) {
  ...
  const cases = result.attempts.map((attempt) => {
    const failure = attempt.passed ? "" : `<failure message="expected ${attempt.expected}, received ${attempt.actual ?? "no response"}">${xml(attempt.error ?? "unexpected status")}</failure>`;
    return `<testcase name="attempt ${attempt.sequence}" classname="${xml(result.name)}" time="${attempt.durationMs / 1e3}">${failure}</testcase>`;
  }).join("");
...
async function main() {
  try {
    ...
    const config = parseTrialConfig(await (0, import_promises.readFile)(configPath, "utf8"), {
      ...process.env,
    ...
    if (input("json"))
      await (0, import_promises.writeFile)(input("json"), `${JSON.stringify(result, null, 2)}
`, "utf8");
    if (input("junit")) await (0, import_promises.writeFile)(input("junit"), junitReport(result), "utf8");
