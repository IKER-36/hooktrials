...
export function readableError(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    api_key_required: 'This action requires a scoped API key.',
    api_key_scope_required: 'This API key does not have the required scope.',
    api_key_not_found: 'That API key no longer exists or has already been revoked.',
    ...
    live_route_required: 'Synthetic forwarding tests require an Observe or Protect route.',
    ingest_token_unavailable: 'This route cannot receive a synthetic event. Recreate it safely.',
    ...
    workspace_membership_required: 'Your account is not assigned to a workspace yet.',
    workspace_read_only: 'Your workspace role is read-only for this action.',
    workspace_role_required: 'An administrator role is required for this action.',
    workspace_member_exists: 'That account is already a member of this workspace.',
    workspace_invite_invalid: 'That invitation is invalid, expired or for another email.',
    workspace_member_not_found: 'That workspace member could not be found.',
    incident_assignee_not_member: 'Choose a member of this workspace as the assignee.',
